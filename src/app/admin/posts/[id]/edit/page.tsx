...
export default function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function loadPost() {
      try {
        ...
        if (!data.success || !data.data) {
          throw new Error('文章数据错误');
        }
        
        const post = data.data;
        
        // 标签处理: 从数组转为逗号分隔的字符串
        const tagsString = post.tags && Array.isArray(post.tags) 
        ...
        setFormData({
          ...
          tags: tagsString,
          original_slug: post.slug || '' // 保存原始slug
        });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // 如果标题改变，可能需要更新slug
    if (name === 'title') {
      // 只有在slug与原始生成的slug匹配时才自动更新
      if (formData.slug === formData.original_slug || 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`更新文章时出错: ${error instanceof Error ? error.message : '请重试'}`);
    } finally {
  ...
  if (isLoading) {
    return <div className="text-center py-10">正在加载文章数据...</div>;
  }
